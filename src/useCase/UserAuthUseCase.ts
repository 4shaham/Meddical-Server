...
class UserAuthUseCase implements IuserAuthUseCase {
    ...
    try {
      ...
      if (values) {
        ...
        if (values.isBlock == true) {
           throw new Errors("this user is blocked",StatusCode.forBidden);
        }
        ...
        if (values.otpVerified == false) {
          ...
          await this.otpServices.sendOtpEmail(
            values.email,
            otp,
            values.userName
          );
  ...
  async verifyOtp(data: otpVerifyData): Promise<resObj | null> {
    try {
      ...
      if (
        otpDatas &&
        otpDatas.otp == data.otp &&
        data.typeOfOtp.trim() == "userEmailVerification"
      ) {
        console.log("hiiiiiiiii shahama it is sucess ");

        let values = await this.userAuthRepository.updateOtpVerified(
          data.email
        );
        ...
        if (values) {
          ...
          return {
            status: true,
            message: "the Otp verification is completed",
            token,
            userData: values,
          };
        }
      ...
      if (
        otpDatas &&
        otpDatas.otp == data.otp &&
        data.typeOfOtp.trim() == "forgotPassword"
      ) {
  ...
  async verifyingUpdatePassword(
    ...
    try {
      ...
      const hashedPassword: string = await this.hashingServices.hashing(
        password
      );
  ...
  async googleAuthenticateUser(data: googleAuthBody): Promise<resObj | null> {
    try {
      ...
      if (!user) {
        ...
        user = await this.userAuthRepository.saveGooogleAuth(
          data.email,
          data.userName,
          data.image
        );
