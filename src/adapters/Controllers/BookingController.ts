...
export default class BookingController implements IBookingController {
  ...
  async cancelTokenBooking(
    ...
    try {
      ...
      const response = await this.bookingUseCase.verifyCancelToken(
        bookingID as string
      );
      res
        .status(StatusCode.success)
        .json({ message: "cancelled succueesffuly" });
    } catch (error) {
      next(error);
    }
  }
  ...
  async findUserBooking(
    ...
    try {
      ...
      const data = await this.bookingUseCase.findBookingDataWithStatus(
        userId,
        statusType
      );
  ...
  async webhook(
    ...
    try {
      ...
      if (result) {
        const response = await this.bookingUseCase.verifyCreateToken(
          userId,
          fees,
          typeOfConsaltation,
          schedulesId,
        ...
        await this.bookingUseCase.createConverasation(
          userId,
          response.doctorId
        );
        await this.bookingUseCase.savePaymentData(
          response.slotId,
          schedulesId,
          transactionId,
          userId,
  ...
  async rescheduleBooking(
    ...
    try {
      ...
      await this.bookingUseCase.isRescheduling(
        slotId,
        slotNumber,
        scheduleId,
        newSlotNumber
